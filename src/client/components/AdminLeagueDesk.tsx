...
const adminViews: Array<[AdminView, string]> = [['season', 'Season'], ['members', 'Members & invites'], ['results', 'Results'], ['people', 'People']];
  ...
  return (
    <section className="admin-desk" aria-labelledby="admin-title">
      ...
      <nav className="admin-tabs" aria-label="Admin workspace">
        {adminViews.map(([key, label]) => <button key={key} className={adminView === key ? 'content-tab content-tab-active' : 'content-tab'} type="button" aria-current={adminView === key ? 'page' : undefined} onClick={() => setAdminView(key)}>{label}</button>)}
      </nav>
      <div className="admin-block">
        ...
        <div className="league-picker">{leagues.map((league) => <button type="button" key={league.id} className={league.id === activeSelectedId ? 'picker-item picker-item-active' : 'picker-item'} onClick={() => { setSelectedId(league.id); onLeagueSelected?.(league); }}><strong>{league.name}</strong><span>{league.seasonName} / {league.status} / {league.visibility.toLowerCase()}</span></button>)}</div>
        <form hidden={adminView !== 'season'} className="compact-form create-league-form" aria-labelledby="create-league-title" onSubmit={createLeague}>
