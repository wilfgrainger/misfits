...
export function AdminLeagueDesk({ user, onLeagueSelected }: AdminLeagueDeskProps) {
  ...
  const createLeague = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError('');
    try {
      const result = await api.createAdminLeague({ name: newName, seasonName: newSeason, maxPlayers: Number(newCapacity), matchesPerPair: Number(newRepeats), targetLegs: Number(newTargetLegs), pointsPerWin: Number(newPointsPerWin) });
  ...
  const saveLeague = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setBusy('save-league');
    try {
      const result = await api.updateAdminLeague(selectedLeague.id, { name: editName, seasonName: editSeason, maxPlayers: Number(editCapacity), matchesPerPair: Number(editRepeats), targetLegs: Number(editTargetLegs), pointsPerWin: Number(editPointsPerWin), status: editStatus });
