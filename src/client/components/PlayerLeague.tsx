...
export function PlayerLeague({ user, league, onUserSaved }: PlayerLeagueProps) {
  ...
  return (
    <section className="player-workspace" aria-labelledby="league-title">
      ...
      {view === 'record' && <form className="result-form" onSubmit={submitResult}><div className="form-heading"><p className="section-kicker">NEW GAME</p><h3>Record your result</h3></div><label htmlFor="opponent">Opponent</label><select id="opponent" value={opponentId} onChange={(event) => setOpponentId(event.target.value)} required><option value="">Choose player</option>{opponents.map((player) => <option key={player.id} value={player.id}>{player.username}</option>)}</select><div className="form-grid"><label htmlFor="your-legs">Your legs<input id="your-legs" type="number" min="0" max={league.targetLegs} value={playerALegs} onChange={(event) => setPlayerALegs(event.target.value)} required /></label><label htmlFor="their-legs">Their legs<input id="their-legs" type="number" min="0" max={league.targetLegs} value={playerBLegs} onChange={(event) => setPlayerBLegs(event.target.value)} required /></label><label htmlFor="your-average">Your average<input id="your-average" type="number" min="0" max="200" step="0.01" value={playerAAverage} onChange={(event) => setPlayerAAverage(event.target.value)} required /></label><label htmlFor="their-average">Their average<input id="their-average" type="number" min="0" max="200" step="0.01" value={playerBAverage} onChange={(event) => setPlayerBAverage(event.target.value)} required /></label></div><button className="primary-button" type="submit" disabled={busyResult === 'new' || opponents.length === 0}>{busyResult === 'new' ? 'Sending' : 'Send for confirmation'}</button></form>}
