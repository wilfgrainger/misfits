...
export function AdminCompetitionDesk({ user, selectedLeagueId, onLeagueSelected, onLeagueCreated, onLeagueChanged }: Props) {
  ...
  const [seasonHealth, setSeasonHealth] = useState<AdminCompetitionHealth | null>(null);
  ...
  const confirmPreviousFocus = useRef<HTMLElement | null>(null);
  ...
  useEffect(() => {
    if (!confirm) {
      if (confirmWasOpen.current) {
        ...
        confirmPreviousFocus.current?.focus();
        confirmPreviousFocus.current = null;
      }
      return;
    }
    if (!confirmWasOpen.current) {
      confirmWasOpen.current = true;
      confirmPreviousFocus.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    }
    const controls = confirmDialogRef.current?.querySelectorAll<HTMLElement>('button:not([disabled])');
    controls?.[0]?.focus();
  ...
  const requestCommitFixtures = () => setConfirm({
    title: 'Commit this fixture schedule?',
    message: 'This publishes the generated schedule to players. Existing played fixtures remain protected.',
    action: commitFixtures,
  });
  ...
  const requestApplyProposal = () => setConfirm({
    title: 'Apply next-season placements?',
    message: 'This creates the reviewed placements in the draft season and marks the movement proposal as applied. Check every destination first.',
  ...
  const requestPlayerUpdate = (player: AdminPlayer, changes: AdminPlayerChanges, title: string, message: string) => setConfirm({
    title,
    message,
    action: () => updatePlayer(player, changes),
  });
  const requestMemberActiveChange = (member: CompetitionMember, active: boolean) => {
    if (active) {
      void setMemberActive(member, true);
      return;
    }
    setConfirm({
      title: `Deactivate ${member.username ?? 'this player'} in the season?`,
      message: 'They will no longer be eligible to submit or resolve results for this league. Existing history remains protected.',
      action: () => setMemberActive(member, false),
    });
  };
  ...
  return <section className="admin-desk admin-competition-desk" aria-labelledby="competition-admin-title">
    ...
    <div role="tabpanel" hidden={task !== 'season'}>
      ...
      {selectedSeason && <section className="admin-block season-health" aria-label="Season health"><div className="section-heading"><div><h3>Season health</h3><p className="form-help">Readiness across every league in {selectedSeason.name}.</p></div>{seasonHealth && <span className="status-label">{seasonHealth.unassignedPlayers === 0 ? 'Ready' : 'Needs placement'}</span>}</div>{seasonHealth ? <div className="health-grid"><span><strong>{seasonHealth.unassignedPlayers}</strong> unassigned</span><span><strong>{seasonHealth.outstandingFixtures}</strong> outstanding</span><span><strong>{seasonHealth.pendingConfirmations}</strong> pending confirmation</span><span><strong>{seasonHealth.disputes}</strong> disputed</span></div> : <p className="empty-message">Checking season readiness…</p>}</section>}
    ...
    <div role="tabpanel" hidden={task !== 'members'}>
      <div className="admin-block"><div className="section-heading"><h3>Unassigned players</h3><span className="count-label">{unassigned.length}</span></div><ul className="admin-list">{unassigned.map((player) => <li key={player.id}><div><strong>{player.username ?? 'Name pending'}</strong><small>{player.email}</small></div><div className="inline-actions"><select aria-label={`League for ${player.username}`} value={assignmentTargets[player.id] ?? ''} onChange={(e) => setAssignmentTargets((current) => ({ ...current, [player.id]: e.target.value }))}>{orderedLeagues.map((league) => <option key={league.id} value={league.id}>{league.name}</option>)}</select><button className="action-button" type="button" onClick={() => void assignPlayer(player)}>Assign {player.username}</button></div></li>)}</ul>{unassigned.length === 0 && <p className="empty-message">Every approved active club player has a league placement.</p>}</div>
      <div className="admin-block"><div className="section-heading"><h3>Season rosters</h3><span className="count-label">{Object.values(membersByLeague).flat().filter((member) => member.active).length}</span></div>{orderedLeagues.map((league) => <div className="roster-group" key={league.id}><h4>{league.name} · {(membersByLeague[league.id] ?? []).filter((member) => member.active).length}/{league.maxPlayers}</h4><ul className="admin-list">{(membersByLeague[league.id] ?? []).map((member) => <li key={member.userId}><div><strong>{member.username ?? 'Name pending'}</strong><small>{member.active ? 'Active' : 'Inactive'}</small></div><div className="inline-actions"><select aria-label={`Move ${member.username}`} value={moveTargets[member.userId] ?? league.id} onChange={(e) => setMoveTargets((current) => ({ ...current, [member.userId]: e.target.value }))}>{orderedLeagues.map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}</select><button className="action-button" type="button" disabled={!member.active || (moveTargets[member.userId] ?? league.id) === league.id} onClick={() => void moveMember(member)}>Move {member.username}</button><button className="action-button" type="button" onClick={() => requestMemberActiveChange(member, !member.active)}>{member.active ? `Deactivate ${member.username}` : `Reactivate ${member.username}`}</button></div></li>)}</ul></div>)}</div>
    ...
    <div role="tabpanel" hidden={task !== 'access'} className="club-access-panel">
      ...
      <section className="admin-block" aria-label="Club members">
        ...
        <ul className="admin-list club-member-list">{approvedPlayers.map((player) => <li key={player.id}><div><strong>{player.username ?? player.email}</strong><small>{player.email} · {player.role} · {player.status}</small><div className="status-chip-row"><span className="status-label">Approved</span><span className="status-label">{player.leagueActive ? 'Season placed' : 'Not placed this season'}</span></div>{player.isMasterAdmin && <small>Protected master administrator</small>}</div>{!player.isMasterAdmin && <div className="inline-actions">{player.role === 'PLAYER' ? <button className="action-button" type="button" onClick={() => requestPlayerUpdate(player, { role: 'ADMIN' }, `Make ${player.username ?? 'this player'} an admin?`, 'Admins can manage club access, seasons and results.')}>Make {player.username} admin</button> : <button className="action-button" type="button" onClick={() => requestPlayerUpdate(player, { role: 'PLAYER' }, `Remove ${player.username ?? 'this player'} admin access?`, 'They will keep club membership but lose administrator controls.')}>Remove {player.username} admin</button>}{player.status === 'ACTIVE' ? <button className="action-button" type="button" onClick={() => requestPlayerUpdate(player, { status: 'SUSPENDED' }, `Suspend ${player.username ?? 'this player'}?`, 'They will be signed out and unable to access club data until reactivated.')}>Suspend {player.username}</button> : <button className="action-button" type="button" onClick={() => void updatePlayer(player, { status: 'ACTIVE' })}>Reactivate {player.username}</button>}</div>}</li>)}</ul>
      </section>
