...
describe('ApiClient admin workspace calls', () => {
  ...
  it('loads club invite metadata without league scope or a raw invite token', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ invites: [{ id: 'invite-1', expiresAt: null, uses: 0, revokedAt: null, createdAt: '2026-08-20T12:00:00.000Z' }] }), { status: 200 }));
    const payload = await new ApiClient().adminClubInvites();
    expect(payload).toMatchObject({ invites: [{ id: 'invite-1', uses: 0 }] });
    expect(payload.invites[0]).not.toHaveProperty('leagueId');
    expect(payload.invites[0]).not.toHaveProperty('tokenHash');
    expect(fetchMock).toHaveBeenCalledWith('/api/admin/club-invites', expect.objectContaining({ credentials: 'include' }));
  });
