...
describe('manual production D1 migration workflow', () => {
  it('requires explicit confirmation and an immutable SHA, verifies it, migrates D1, and never deploys application code', () => {
    ...
    expect(workflow).toContain('npx wrangler d1 execute misfits --remote');
    expect(workflow).toContain('PRAGMA table_info(users)');
    expect(workflow).toContain("name='club_invites'");
    expect(workflow).toContain('SELECT club_status, COUNT(*) FROM users GROUP BY club_status');
    expect(workflow).toContain('SELECT visibility, COUNT(*) FROM leagues GROUP BY visibility');
