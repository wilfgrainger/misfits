...
| **ADM-070** | As an administrator, I want equal standings positions resolved by the approved rule so that competition ordering is explainable and deterministic. | Competitive order is points, then total legs won, then head-to-head points; two-player head-to-head aggregates their confirmed meetings; 3+ ties use a mini-table; players still equal share rank; deterministic presentation never becomes a competitive tie-break; promotion/relegation surfaces a boundary ambiguity rather than guessing. | **MISSING · P0** |
