...
**Date:** 20 August 2026  
**Release commit:** `0449ae8632f601ede2123d5cdac758e00ac5ea71` (`fix: hide deleted results and clear stale confirmation`)
**Cloudflare Worker version:** `bdf5114c-7706-4969-a16b-0bfd13daa996`
**Live origin:** `https://darts.graingers.agency`

## Source and automated verification

- `npm test`: 17 test files, 62 tests passed.
...
- After the final server hardening deployment, `GET /api/health`, the public league list, the redacted player endpoint, the seeded standings/results endpoints, and the unauthenticated admin guard were rechecked; the public endpoints returned `200`, the admin guard returned `401`, and the legacy workers.dev hostname returned `404`.
