...
- `npm test`: 17 test files, 57 tests passed.
...
Focused coverage includes Google picture persistence, profile ownership/validation, league create/edit authorization, invite hashing/join/capacity/revocation, player-only result submission, average rounding, pair limits, confirmation/dispute transitions, confirmed-only standings, admin result correction/deletion/audit records, client API paths and mobile player/admin rendering.
